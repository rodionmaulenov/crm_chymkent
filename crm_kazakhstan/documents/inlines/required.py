from documents.models import RequiredDocument
...
class DocumentRequiredInline(admin.TabularInline):
    ...
    max_num = len(RequiredDocument.RequiredDocumentChoice.choices)

    class Media:
        js = ('documents/js/remove_duplicates_required.js', 'documents/js/redirect_on_required_docs.js')
    ...
    def get_fields(self, request, obj=None):
        if 'documents' in request.GET:
            ...
            return 'file', 'get_html_photo', 'note', 'date_create', 'download_link'
    ...
    def download_link(self, obj):
        if obj.file:
            url = reverse('admin:document_required_download', args=[obj.id])
            return format_html(
    ...
    def get_html_photo(self, obj):
        if obj.file:
            ...
            if file_url.endswith('.pdf'):
                # Embed the PDF in an iframe within a container
                return '-'
            else:
                # Display the image
                return mark_safe(f"""
