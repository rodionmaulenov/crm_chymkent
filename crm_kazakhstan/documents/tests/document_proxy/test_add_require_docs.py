...
from documents.models import DocumentProxy, RequiredDocument
from mothers.models import Mother, Stage

User = get_user_model()
Stage: models
Mother: models
RequiredDocument: models


class AddRequireDocsTest(TestCase):
    def setUp(self):
    ...
    def test_add_document_link(self):
        ...
        add_new = self.document_admin.add_require_docs(self.mother)
        mother_pk = self.mother.pk
        required = (
            f'<a class="add-change-link " href="/admin/documents/documentproxy/{mother_pk}/change/?mother={mother_pk}&amp;documents=required">'
        )
    ...
    def test_if_user_has_only_view_perm(self):
        ...
        mother_pk = self.mother.pk
        required = (
            f'<a class="add-change-link disabled-link" href="/admin/documents/documentproxy/{mother_pk}/change/?mother={mother_pk}&amp;documents=required">'
        )
        self.assertIn(required, add_new)
    ...
    def test_add_change_icon(self):
        ...
        for integer in range(1, 8):
            ...
            RequiredDocument.objects.create(mother=self.mother, file=mock_file, note=f'some{integer}', title=f'title{integer}')
