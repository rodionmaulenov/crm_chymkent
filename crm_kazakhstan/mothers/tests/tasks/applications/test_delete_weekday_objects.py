...
from mothers.tasks import delete_weekday_objects
from freezegun import freeze_time


class DeleteWeekdayObjectsTestCase(TestCase):
    ...
    @freeze_time("2024-07-08 00:00:00")
    def test_delete_objs_from_previous_monday_to_friday_inclusive(self):
        ...
        self.assertFalse(Mother.objects.filter(id=monday.id).exists())
        self.assertFalse(Mother.objects.filter(id=tuesday.id).exists())
        self.assertFalse(Mother.objects.filter(id=wednesday.id).exists())
        self.assertFalse(Mother.objects.filter(id=thursday.id).exists())
        self.assertFalse(Mother.objects.filter(id=friday.id).exists())

        # Check that the instances created on the weekend and those without null fields are still there
        self.assertTrue(Mother.objects.filter(id=saturday.id).exists())
        self.assertTrue(Mother.objects.filter(id=sunday.id).exists())
